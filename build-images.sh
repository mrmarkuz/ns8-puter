...
# Reuse existing nodebuilder-puter container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-puter; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-puter -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=ghcr.io/heyputer/puter:latest" \
    "${container}"
# Commit the image
